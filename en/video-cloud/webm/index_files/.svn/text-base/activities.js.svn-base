/*
$Id: activities.build.js 29780 2010-01-11 20:48:41Z santep $
...
TimesPeople.TwitterTool = {
  ...
  initialize: function(element, position, x, y) {
  ...
  onClick: function(event) {
    if (this.user.profile.tw_id) {
      ...
      window.location.replace(TimesPeople.Config.get_started_uri + '/twitter?return_url=' + encodeURIComponent(window.location.href));
    }
  },
  
  shortenURL: function() {
    var url = TimesPeople.Page.getUrl('tptw');
    var self = this;
  ...
  onShortenURL: function(response) {
    ...
    if (spinner) spinner.hide();
    var headline = TimesPeople.Page.getTitle();
  ...
  onSubmit: function(event) {
    ...
    new TimesPeople.Activity({
      ...
      object: TimesPeople.Page.getTitle(),
      object_url: TimesPeople.Page.getUrl(),
      object_thumbnail_url: TimesPeople.Page.getThumbnail(),
      object_note: text
    });
    this.disabled = true;
    event.stop();
...
*/
        if (TimesPeople.Page.group!=undefined) {
            ...
            switch (TimesPeople.Page.group){
                case "article": 
                    this.addtwitter(TimesPeople.Page.group);
                    this.addRecommend();
                    this.addRecommend({key: "div.nextArticleLink", target: "div.nextArticleLink", type: "span", style: "float:right;", position: "first", html: "<a class='timespeople_recommend_link' style='cursor:pointer;background-image:url(__ICON__); background-repeat:no-repeat; padding:0 0 3px 20px; font-size:1em;'>__LABEL__</a>", trackingID: 'Bottom'});
                    this.addComments();
                    break;
                case "topic":
    ...
    addtwitter: function(pageType) {
      ...
      switch(pageType) {
        ...
        case 'multimedia':
          ...
          selector = config.key;
          if ($$(selector)[0]) {
            TimesPeople.TwitterTool.initialize($$(config.target)[0], position);
          }
          break;
        case 'blog post':

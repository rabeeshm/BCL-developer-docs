...
echo --------------------------
TOKEN=$(curl -s -k --data "grant_type=client_credentials" https://oauth.staging.brightcove.com/v3/access_token --header "Content-Type: application/x-www-form-urlencoded" --user "$CLIENT_ID:$CLIENT_SECRET" | sed -E 's/.*access_token\"\:\"([^\"]+)\".*/\1/');

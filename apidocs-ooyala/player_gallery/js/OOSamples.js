...
var OOSamples = {
    ...
    contentTemplate: function(){
       $("body").append("<div class='ui-layout-north'>"+
               "<div id='logo-wrapper'><a href='http://www.ooyala.com/' id='logo'><img src='imgs/logo_landingpage.png' alt='Ooyala' class='logo'/></a> <div style='position: fixed;top: 23px;left: 228px;color: #444;font-size:26px;'>PLAYER GALLERY</div></div>"+
                ...
                "<div id='controlsHolder'>"+
                    "<nav>"+
                        "<ul>"+
                            ...
                            "<li><div id='hideMBus' class='menu' tooltip='Message Bus' icon='fa-list-alt'><i class='fa fa-list-alt fa-lg'></i></div></li>"+
...
function URLEditor(val,width,height,UIElement){
    ...
    this.loadURL = function(){
        if (this.evalURL()){
            var debugParam = (this.code().indexOf("?")!=-1) ? "&debug=true":"?debug=true";
            var url = this.code().concat(debugParam);
...
function JSEditor(val,width,height,UIElement){
    ...
    this.enableMessageBusConsole = function (){
        if (!this.messageBus){
            this.messageBus = true;
            var code = this.code();
            var start = code.indexOf("Player.create");
            var playerSection = code.substring(start,code.length);
            var curlyPosition = start + playerSection.indexOf("{");

            var messageBus = "{\n onCreate: function(player) {\nwindow.messageBus = player.mb;\n}";
            var subscribeCode = "\n\n//Message Bus is subscribed to all events \nOO.ready(function() { window.messageBus.subscribe('*' , 'example', function(eventName) { console.log(eventName);logMessageBus(eventName);});});"

            var code = code.substring(0,curlyPosition) + messageBus + code.substring(curlyPosition+1, code.length) + subscribeCode;
...
function HTMLEditor(val,width,height,UIElement){
    ...
    this.messageBus = false;
    this.loadCode = function(){
        var code = (this.messageBus)? this.code()+"<div class='messageBus'><h2 id='videoTitle'>Message Bus</h2></br></br>"+
                                                "<div><table boder=0 cellpadding=10 cellspacing=0></table></div></div>":this.code();
        $("#playerArea").html(code); // Loads the HTML code into "playerArea" DIV
    }
    this.enableMessageBusConsole = function(){
        this.messageBus = true;
    }
...
$(document).on("click","#enableDebug",function(){
...
$(document).on("click","#hideMBus",function(){
    /*
@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
    */
    ...
    if (OOSamples.finalCode.hidden){
        $("#hideDescription").removeClass("active");
        OOSamples.contentLayout.hide('south');

       OOSamples.playerCode.enableMessageBusConsole();
       OOSamples.HTMLCode.enableMessageBusConsole();
...
var counter = 1;
function logMessageBus(message){
    var d = new Date();
    d.getTime();
    var c = (counter%2!=0)?"shaded":"";
    $(".messageBus table").append("<tr class='"+c+"'><td>"+ counter +"</td><td>"+ d.getHours()+":"+d.getMinutes()+":"+d.getSeconds()+"."+d.getMilliseconds() +"</td><td>"+ message +"</td></tr>");
    counter++;
}

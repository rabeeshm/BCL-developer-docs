! function($) {
  var updateRate = function(new_val) {
    $(".js-current-rating").removeClass().addClass("current-rating js-current-rating current-rating--" + new_val)
  };
  $(document).on("ready", function() {
    ...
    "undefined" != typeof Storage && "undefined" == typeof localStorage[currentURL] && (disabled = !1), $.post("/get/rating", params, function(res) {
      currentRating = res.rating, updateRate(currentRating)
    }, "json"), $(".js-star-rating").on("change", "input", function() {
      disabled || (localStorage.setItem(currentURL, 1), disabled = !0, currentRating = this.value, updateRate(currentRating), params = {
        url: currentURL,
        rating: currentRating
      }, $.post("/post/rating", params))
    }), $(".star-rating__input").on("mouseenter", function() {
      disabled || updateRate(this.value)
    }).on("mouseleave", function() {
      disabled || updateRate(currentRating)
    })
  })
}(jQuery);;
! function($) {
  $(document).on("ready", function() {
    var $body = $("body"),
      BODY_CLASS = "ooyala-help-processed";
    if ($(".dita-content a").each(function() {
        var ele = $(this);
        ele.attr("href").indexOf("#") < 0 && ele.attr("target", "_blank")
      }), !$body.hasClass(BODY_CLASS)) {
      var $searchForm = $(".l-header .google-cse"),
        $searchInput = $("#edit-search-block-form--2", $searchForm),
        $communityFeed = $("#community-feed-block"),
        $searchSubmit = $('input[type="submit"]', $searchForm);
      if ($searchInput.attr("placeholder", "Search Help"), $searchSubmit.on("click", function(e) {
          "" === $searchInput.val() && (e.preventDefault(), $searchForm.toggleClass("active"))
        }), $communityFeed.length) {
        var scr = document.createElement("script"),
          host = "http://community.ooyala.com";
        scr.setAttribute("async", "true"), scr.type = "text/javascript", scr.src = host + "/t5/activecast/latestthreadspage?activecast.parent_element_id=lia-ac-lt-backlot_developers&profile.language=en", ((document.getElementsByTagName("head") || [null])[0] || document.getElementsByTagName("script")[0].parentNode).appendChild(scr), scr.onload = function() {
          LITHIUM.ActiveCast.getLatestThreads("backlot_developers", {
            parentElementId: "community-feed-block",
            displayNodeLabel: !1,
    ...
    var main_resize = function() {
      $(".content .dita-content table .codeblock").each(function() {
        var curr = $(this);
        curr.width("10px"), curr.width(curr.parent().width())
      }), $(".content .dita-content table").each(function() {
        var curr = $(this);
        curr.parent().width() > curr.width() && curr.width(curr.parent().width())
      })
    };
...
! function(e) {
  ...
  function l(e, t, a, i) {
    ...
    var B = "",
      M = 0;
    try {
      for (var I, j, O = 0; E.t.lastIndex = O, I = E.t.exec(t), I;) j = m(t.substr(O, I.index - O), I[0]), O = I.index + j;
...
function isInMobileOrTablet() {
  return isMobile.any || isMobile.apple.tablet || isMobile.android.tablet || isMobile.seven_inch
}! function(i) {
  var e = /iPhone/i,
    n = /iPod/i,
    o = /iPad/i,
...
var smoothScroll = function(hash, offset) {
  offset = "undefined" == typeof offset ? 300 : offset, jQuery("html, body").stop().animate({
...
! function($) {
  $(document).on("ready", function() {
    var height = $(window).height(),
      ...
      $aside = $(".l-content .side-bar"),
      $content = $(".l-content article");
    $('<div class="toggler"></div>').insertAfter($("aside ul.menu > li.expanded > a")), $(".expanded:not(.active-trail)").addClass("closed"), $(".expanded.active-trail").addClass("open"), $(".expanded > .toggler").click(function(e) {
      e.preventDefault(), $(this).parent().toggleClass("open closed")
    });
    var last_separator = $(".easy-breadcrumb_segment-separator").last();
    "" === last_separator.next().html() && last_separator.remove(), $(".hamburger").click(function(e) {
      e.preventDefault(), $(this).toggleClass("is-active"), $("body").toggleClass("open-mobile")
    }), $(".menu-title").click(function() {
      $(this).toggleClass("open"), $(".l-main aside").slideToggle()
    }), $(window).scroll(function() {
      if (width > 1024) {
        var scrollTop = dheight - height - fheight - $(window).scrollTop();
        0 > scrollTop ? $("aside").addClass("short") : $("aside").removeClass("short")
      }
    });
    var aside_resize = function() {
      var asideHeight = $aside.height(),
        contentHEight = $content.height();
      asideHeight > contentHEight && $content.height(asideHeight), $aside.addClass("fix")
    };
    $(window).resize(function() {
      aside_resize()
    }), aside_resize();
    var active_option = $(".active-trail.active");
    if (active_option.parent().addClass("active-parent"), active_option.length) {
      var pos_aside = $aside.offset(),
